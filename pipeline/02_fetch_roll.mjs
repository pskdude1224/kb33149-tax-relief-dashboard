...
//
// Reads data/raw/parcels.json (combined multi-zip) and writes data/raw/roll.json.
// Resumable: re-runs skip folios already captured unless --fresh is passed.
//
...
const ROLL_PATH = join(RAW_DIR, "roll.json");
...
const parcels = JSON.parse(readFileSync(join(RAW_DIR, "parcels.json"), "utf8"));
const folios = parcels.map((p) => String(p.FOLIO));
...
function extract(folio, j) {
  ...
  return {
    folio, ok: !invalid, roll_year: rollYear || null,
    ...
    homestead,
    just_value: a ? a.TotalValue : null,
    assessed_value: a ? a.AssessedValue : null,
    ...
    exemptions: benefits.filter((b) => b.Type === "Exemption").map((b) => ({ desc: b.Description, value: b.Value })),
...
const years = {}, munis = {}; let hs = 0;
for (const r of valid) {
  years[r.roll_year] = (years[r.roll_year] || 0) + 1;
  munis[r.municipality || "(none)"] = (munis[r.municipality || "(none)"] || 0) + 1;
  if (r.homestead) hs++;
}
writeFileSync(join(RAW_DIR, "roll_meta.json"), JSON.stringify({
  fetched_at: new Date().toISOString(), source: PROXY, operation: "GetPropertySearchByFolio",
  total_folios: folios.length, captured: results.length, valid: valid.length,
  invalid_or_failed: results.length - valid.length, homestead_count: hs,
  roll_year_distribution: years, municipality_distribution: munis,
}, null, 2));

console.log(`\n=== DONE === captured ${results.length}, valid ${valid.length}, homestead ${hs}`);
console.log("roll years:", years); console.log("municipalities:", munis);
